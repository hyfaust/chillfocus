...
```css
.trackListWrap {
  max-height: 220px;          /* 限制显示高度 */
  overflow-y: auto;           /* 超出滚动 */
  border: 1px dashed var(--border);  /* 虚线边框 — 暗示可拖拽区域 */
}
```

曲目列表使用 **Dashed Border（虚线边框）** 标识可拖拽区域，最大高度 220px 约显示 6 首曲目。
...
使用 **Sticky Positioning（粘性定位）** 确保按钮始终吸附在列表底部可见位置。
