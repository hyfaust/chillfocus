...
**问题**：快捷键设置采用 `display: flex` + `justify-content: space-between` 布局，三列（功能名、局部快捷键、全局快捷键）的宽度随内容变化。不同行的快捷键按键宽度不一致（如 "Space" 比 "N" 宽），导致中间列在垂直方向上参差不齐。
...
- 第一列 `1fr` 自适应剩余空间（功能名称）
- 第二、三列 `100px` 固定宽度（局部/全局快捷键）
- 按键按钮 `width: 80px` 统一宽度，内容居中
- 列头行与数据行共享同一个 `.shortcutRow` class，自动对齐

> **教训**：需要多行多列对齐的表单/列表布局，优先使用 CSS Grid 固定列宽，而非 Flex 的 space-between。Flex 适合一维弹性布局，Grid 适合二维表格布局。
