...
项目同时支持 **Web 浏览器** 和 **Tauri 桌面端** 两种运行环境。Tauri 使用系统原生 WebView 渲染前端界面，无需打包 Chromium 内核，体积更小。所有 UI 组件统一基于 Web 技术栈（React + CSS Modules），确保两个平台的视觉表现一致。
...
Tauri 桌面端默认窗口尺寸为 **1200×800**，最小窗口限制为 **800×600**（`tauri.conf.json`）。窗口启动时自动居中，用户可自由调整大小。
...
在 `768px` 断点处，双栏 **Grid** 切换为单列，面板移除最大高度限制，变为自然高度。
...
进度环使用 `viewBox="0 0 200 200"` 的 SVG 画布，圆半径 `r=90`，整体渲染尺寸为 **200px × 200px**。
...
两种模式可组合使用，适用于极简专注场景。设置面板中通过开关按钮切换。

### 8.7 自定义背景图片

支持自定义背景图片，半透明叠加在渐变色上（`opacity: 0.35`）。图片通过 `ImageCropper` 组件以 **3.75:1 固定比例** 裁剪后以 data URL 存储，匹配番茄钟容器的宽高比。裁剪器支持拖拽移动裁剪区域和四角手柄缩放。
...
| `loop-single` | 🔂 | 单曲循环 |
| `shuffle` | 🎲 | 随机播放 |
| `single` | 1️⃣ | 单曲播放，播完暂停（不自动下一首） |

模式偏好通过 `localStorage` 持久化，刷新后自动恢复。

### 10.5 添加音乐下拉菜单

点击"添加音乐"按钮展开下拉菜单，提供两种添加方式：

| 方式 | 说明 |
|------|------|
| **📁 本地文件** | 通过 `<input type="file">` 选择本地音频文件（支持多选），Web 端读取为 Blob，Tauri 端通过原生文件对话框读取文件路径 |
| **URL 导入** | 输入音频 URL 和可选曲目名称，回车或点击"添加"按钮导入远程音频 |
...
下拉菜单使用较深的背景色 `#1e1e32`（比面板更深），配合 **8px 偏移 + 24px 模糊** 的深层阴影，与面板形成明确的层级关系。两种方式之间以分割线分隔。
...
```css
.item {
  border-left: 3px solid task-color;  /* 左侧彩色边框 */
}
...
颜色通过两处视觉元素体现：
- **左侧 3px 彩色边框**：标识任务归属颜色
- **勾选框颜色**：完成态勾选框使用对应任务颜色填充

添加任务时可通过色块按钮选择颜色，任务上的色点可循环切换。
...
预览方框带有脉动动画（opacity 0.6→1），跟随鼠标位置移动，提供即时的放置反馈。
...
z-index   元素                   语义
────────────────────────────────────────────
  110     ImageCropper           背景图片裁剪器（模态覆盖）
  100     设置面板 / 拖拽预览     番茄钟设置面板 / 便签拖拽预览
   60     下拉菜单               音乐播放器下拉菜单
   50     便签图标               全局操作入口
   40     浮动便签层              便签 overlay（fixed）
    5     固定便签层              便签 overlay（absolute）
    5     设置按钮                容器内交互元素
    2     回到顶部按钮 / Resize   容器内辅助元素
    1     内容层 (.content)       主要内容
    0     可视化层                底层装饰
   —      背景层                  最底层
```
